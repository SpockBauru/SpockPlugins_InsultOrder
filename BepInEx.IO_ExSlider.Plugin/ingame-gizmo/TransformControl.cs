...
namespace mattatz.TransformControl {
	...
	public class TransformControl : MonoBehaviour {
        ...
        // add by nn@
        protected const float HANDLER_SIZE_DEFAULT = 0.2f;
        protected float HANDLER_SIZE = HANDLER_SIZE_DEFAULT;
        protected float scaleTranslate = 1f;
        protected float GizmoScale = 1f;
        ...
        const int SPHERE_RESOLUTION_DEF = 32;
        protected int SPHERE_RESOLUTION = SPHERE_RESOLUTION_DEF;
	    ...
	    void Awake() {
	        ...
	        GetCircumference(1f, SPHERE_RESOLUTION, out circumX, out circumY, out circumZ);
	    }

        // add by nn@
#if true
        public void SetHandlerSize(float size)
        {
            GizmoScale = (size / HANDLER_SIZE_DEFAULT);
        }
        ...
        public void SetSphereSize(float size)
        {
            ...
            // 大きくしたら点数＝解像度を増やさないとクリックに反応しにくい
            // num of points
            var num = SPHERE_RESOLUTION_DEF;
            if ((size / 1f) > 2f)   //  defsize = 1f
            {
                num *= ((int)size >> 1) << 1;
            }
...
#else
        // GetTranformに気づく前に力技で何とかしようとしてた名残り。現行で問題なさそうなら削除
        public void SetHandlerSize(float size)
        {
            //HANDLER_SIZE = size;
            //cone = CreateCone(5, 0.1f, HANDLER_SIZE);
            ...
            SetMatricesScale(size / HANDLER_SIZE_DEFAULT);
        }

        private void SetMatricesScale(float value)
        {
            scaleTranslate = value;

            matrices = new Matrix4x4[] {
                Matrix4x4.TRS(Vector3.right * value, Quaternion.AngleAxis(90f, Vector3.back), Vector3.one * value),
                Matrix4x4.TRS(Vector3.up * value, Quaternion.identity, Vector3.one * value),
                Matrix4x4.TRS(Vector3.forward * value, Quaternion.AngleAxis(90f, Vector3.right), Vector3.one * value)
            };
        }
        ...
        public void SetSphereSize(float size)
        {
            ...
            GetCircumference(size, SPHERE_RESOLUTION = num, out circumX, out circumY, out circumZ);
        }
#endif
        public void SetThrethold(float value)
        {
            THRESHOLD = value;
        }
        ...
        public bool isDragging
        {
            get
            {
                return dragging & selected != TransformDirection.None;
            }
        }
        ...
        */
        ...
        public void Control () {
            ...
            //if (Input.GetMouseButtonDown(0) || Input.GetMouseButtonDown(1) || Input.GetMouseButtonDown(2)) { //by nn@ 
                dragging = true;
	            start = Input.mousePosition;
	            prev = new TransformData(transform);
                Pick();
	        } else if (Input.GetMouseButtonUp(0)) {
            //} else if (Input.GetMouseButtonUp(0) || Input.GetMouseButtonUp(1) || Input.GetMouseButtonUp(2)) { //by nn@ 
                dragging = false;
				selected = TransformDirection.None;
	        }
        ...
        Matrix4x4 GetTranform()
        {
			...
			return Matrix4x4.TRS(transform.position, global ? Quaternion.identity : transform.rotation, Vector3.one * scale * GizmoScale);
        }
	    ...
	    bool PickOrthogonal (Vector3 mouse) {
	        ...
	        var right = cam.WorldToScreenPoint(matrix.MultiplyPoint(Vector3.right * scaleTranslate)).xy() - origin;
	        var rightHead = cam.WorldToScreenPoint(matrix.MultiplyPoint(Vector3.right * (1f + HANDLER_SIZE) * scaleTranslate)).xy() - origin;
	        var up = cam.WorldToScreenPoint(matrix.MultiplyPoint(Vector3.up * scaleTranslate)).xy() - origin;
	        var upHead = cam.WorldToScreenPoint(matrix.MultiplyPoint(Vector3.up * (1f + HANDLER_SIZE) * scaleTranslate)).xy() - origin;
	        var forward = cam.WorldToScreenPoint(matrix.MultiplyPoint(Vector3.forward * scaleTranslate)).xy() - origin;
	        var forwardHead = cam.WorldToScreenPoint(matrix.MultiplyPoint(Vector3.forward * (1f + HANDLER_SIZE) * scaleTranslate)).xy() - origin;
	    ...
	    void GetCircumference (float size, int resolution, out List<Vector3> x, out List<Vector3> y, out List<Vector3> z) {
	        x = new List<Vector3>();
	        y = new List<Vector3>();
	        z = new List<Vector3>();
	        ...
	        for(int i = 0; i < resolution; i++) {
	            ...
	            x.Add(new Vector3(0f, size * Mathf.Cos(r), size * Mathf.Sin(r)));
	            y.Add(new Vector3(size * Mathf.Cos(r), 0f, size * Mathf.Sin(r)));
	            z.Add(new Vector3(size * Mathf.Cos(r), size * Mathf.Sin(r), 0f));
	        }
	    }
	    ...
	    void DrawTranslate () {
	        ...
	        DrawLine(Vector3.zero, Vector3.right * scaleTranslate, color);
	        DrawMesh(cone, matrices[0], color);

	        // y axis
	        color = selected == TransformDirection.Y ? colors[3] : colors[1];
	        DrawLine(Vector3.zero, Vector3.up * scaleTranslate, color);
	        DrawMesh(cone, matrices[1], color);

	        // z axis
	        color = selected == TransformDirection.Z ? colors[3] : colors[2];
	        DrawLine(Vector3.zero, Vector3.forward * scaleTranslate, color);
